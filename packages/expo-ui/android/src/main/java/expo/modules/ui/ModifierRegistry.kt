...
internal enum class BlendModeType(val value: String) : Enumerable {
  CLEAR("clear"),
  SRC("src"),
  DST("dst"),
  SRC_OVER("srcOver"),
...
@OptimizedRecord
internal data class RecordLayerParams(
  @Field val name: String = "",
  @Field val blendMode: BlendModeType = BlendModeType.SRC_OVER
) : Record

@OptimizedRecord
internal data class DrawLayerParams(
  @Field val name: String = ""
) : Record
...
object ModifierRegistry {
  ...
  private fun registerBuiltInModifiers() {
    ...
    register("recordLayer") { map, _, _, _ ->
      val params = recordFromMap<RecordLayerParams>(map)
      val store = LocalNamedGraphicsLayers.current ?: return@register Modifier
      val layer = remember(store, params.name) { store.getOrCreate(params.name) }
    ...
    register("drawLayer") { map, _, _, _ ->
      val params = recordFromMap<DrawLayerParams>(map)
      val store = LocalNamedGraphicsLayers.current ?: return@register Modifier
      val name = params.name
      Modifier.drawWithContent {
        drawContent()
        store.get(name)?.let { drawLayer(it) }
      }
    }
